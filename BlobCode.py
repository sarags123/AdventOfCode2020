...
def satellite_init():
    ...
    satellite_file = open("satellite_id.txt", "r")

    satellite_line = satellite_file.readlines()
    
    satellite_list = satellite_line[0].split(" ")
    

def get_satellite_id(sequence):
    global satellite_list
    #read satellite id from sat file
    #update internal satellite id

    
    return satellite_list[sequence]

def radio_read(sequence):
    # Read data from satellite and return it
    # receive
    # rx_file to get data from the satellite and return data
    #instead of opening the file name use self.rx_file
    receiver = (103,101,102,103,104,105)
    data = "test"
    
    
#     rx_file.close()
    return (data, receiver[sequence])

def radio_send(receiver, data):
...
for i in range(0, 5):
    ...
    sat_id = int(get_satellite_id(i))
#    print(sat_id)
    (data, receiver) = radio_read(i)
...
#           print(send_blob_id_list)
           radio_send(sat_id, get_blob(send_blob_id_list))

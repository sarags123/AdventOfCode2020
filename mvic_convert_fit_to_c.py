...
# 
# Usage:    python mvic_convert_fit_to_c.py [directory of FITS files]
# Example:  python mvic_convert_fit_to_c.py quaoar_133g_L1_30s_first10
# 
...
def get_instrument_type(src_file): 
    ...
    if "lor" in instrument:
        print ("LORRI instrument type detected, this script will not work. Use convert_fit_to_c.py instead")
        instrument_type = "LORRI_DT2"
        IMAGE_X_LEN = 257
        IMAGE_Y_LEN = 256

    elif "mvi" in instrument:
        print ("MVIC instrument type detected, proceeding...")
        instrument_type = "MVIC_DT4"
        IMAGE_X_LEN = header.get('NAXIS2')
        IMAGE_Y_LEN = header.get('NAXIS1')
    else: 
...
def create_c_header_from_fits_files(src_file_list, directory_name, IMAGE_X_LEN, IMAGE_Y_LEN, instrument_type):
    ...
    if instrument_type == "LORRI_DT2":
        c_str += "#define IMAGE_COUNT " +  str(len(src_file_list)) + "\n"
    if instrument_type == "MVIC_DT4":
        c_str += "#define IMAGE_COUNT " +  str(len(src_file_list)*11) + "\n"
    c_str += "#define INSTRUMENT_TYPE " + instrument_type + "\n"
    c_str += '\n'
    c_str += 'void PrintDataSet(void)\n{\n' + (" " * indent) + 'printf( \"Data Set: %s\\n\", __FILE__);\n}'    
    c_str += '\n'
    c_str += '\n'
    ...
    for file_count, src_file in enumerate(src_file_list):
        # read both data and header
        data, header = fitsio.read(src_file, header=True)
        # Create and open a .h file to output the scanlines to
        output = data_to_c(data, file_count)
        # If you desire to only do a single image in that file...
        # output = data_zero_to_c(data)
        # Write once per file
        f.write(output)
...
def data_to_c(data, file_count): # pass in a single mvic fit file-- this works but creates a MONSTER file
    c_str = ""
    c_str += (" " * indent) + '{\n'
    print( "File number " + str(file_count) + ", processing total image number" )
    for image in range(len(data)): # index for each of the (currently 11) images in there
        string = str(image+(file_count*11)) + " ..."
        print (string)
        sys.stdout.flush()
        for scanline in range(len(data[image])): # index for each of the 128 scanlines
            c_str += '// Now starting scanline %d of image %d\n' % (scanline, (image + (file_count*11)))
            c_str += (" " * indent * 2) + '{\n' # beginning of scanline 
            for pixel in range(len(data[image][scanline])):
                if (pixel != len(data[image][scanline])):
                    # If this is not the last pixel...
                    c_str += (" " * indent * 3) + ("0x%04X,\t\t// pixel %d\n" % (np.uint16(data[image][scanline][pixel]), pixel))
                else:
...
# process all the fit file data into a string for writing out
def data_zero_to_c(data): # pass in a single mvic fit file-- this works but creates a MONSTER file
    indent = 3
    c_str = ""
    c_str += (" " * indent) + '{\n'
    # for image in range(len(data)): # index for each of the (currently 11) images in there
    for scanline in range(len(data[0])): # index for each of the 128 scanlines
        c_str += '// Now starting scanline %d of image 0\n' % (scanline)
        c_str += (" " * indent * 2) + '{\n' # beginning of scanline 
        for pixel in range(len(data[0][scanline])):
        
            print( "Image " + str(image) + " Scanline " + str(scanline) + " Pixel " + str(pixel) )

            if (pixel != len(data[0][scanline])):
                # If this is not the last pixel...
                c_str += (" " * indent * 3) + ("0x%04X,\t\t// pixel %d\n" % (np.uint16(data[0][scanline][pixel]), pixel))
                ...
                print( "Last Pixel of image " + str(image) + ", scanline " + str(scanline) )
...
directory_name = sys.argv[1].rstrip("/") # get name of directory containing the files, remove trailing slash
